...
echo [ai-emotion] Starting backend (reload) and frontend (dev)...

start "ai-emotion server" cmd /k "cd /d \"%ROOT%\" && call .venv\\Scripts\\activate.bat && python -m uvicorn server.app.main:app --reload --host 127.0.0.1 --port 8000"
start "ai-emotion client" cmd /k "cd /d \"%ROOT%\\client\" && set NEXT_PUBLIC_API_BASE=http://127.0.0.1:8000 && set NEXT_PUBLIC_WS_URL=ws://127.0.0.1:8000/ws/events && npm run dev"
