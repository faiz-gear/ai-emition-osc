...
class QwenVoiceProcessor:
    ...
    def parse_emotion_output(self, output: str) -> tuple:
        """解析LLM输出格式：emotion|value，针对Qwen2.5优化"""
        try:
            # 清理输出，移除可能的额外内容
            output = output.strip()
            lines = output.split("\n")
            ...
            for line in lines:
                ...
                if "|" in line:
                    ...
                    if match:
                        ...
                        return emotion, min(1.0, max(0.1, value))
            ...
            emotion_keywords = {
                "joy": ["joy", "喜悦", "开心", "高兴", "快乐"],
                "sadness": ["sadness", "悲伤", "难过", "失落", "沮丧"],
                "anger": ["anger", "愤怒", "生气", "烦躁", "恼火"],
                "fear": ["fear", "恐惧", "担心", "害怕", "焦虑"],
                "surprise": ["surprise", "惊讶", "意外", "震惊", "诧异"],
                "disgust": ["disgust", "厌恶", "反感", "恶心", "讨厌"],
                "trust": ["trust", "信任", "信赖", "相信", "依靠"],
                "anticipation": ["anticipation", "期待", "期望", "盼望", "渴望"],
            }

            for emotion, keywords in emotion_keywords.items():
                for keyword in keywords:
                    if keyword in output_lower:
                        return emotion, 0.7  # 默认中等强度

            return "joy", 0.5  # 最终默认返回
